#!/usr/bin/env python3
"""
Duplicate Segments Cleanup Script for Giggles Application

This script removes duplicate/overlapping audio segments from the database.
It keeps the most recent segment for each time period and removes older duplicates.
...
class DuplicateSegmentCleaner:
    def __init__(self):
        ...
        print("🧹 Giggles Duplicate Segments Cleaner")
        print("=" * 50)
    ...
    def select_segments_to_keep(self, overlapping_group: List[Dict]) -> List[Dict]:
        """Select which segments to keep from an overlapping group."""
        if len(overlapping_group) <= 1:
            return overlapping_group
        
        # Sort by priority:
        # 1. Processed segments (keep processed over unprocessed)
        # 2. Most recent creation time
        # 3. Longest duration (more complete data)
        ...
        def segment_priority(segment):
            ...
            return (processed_score, time_score, duration)
    ...
    def cleanup_duplicates(self, user_id: str) -> Dict:
        ...
        for i, (original, duplicates) in enumerate(overlaps):
            ...
            for j, segment in enumerate(all_segments):
                ...
                print(f"  {j+1}. {status} {segment['id'][:8]}... | {start_time} → {end_time}")
            ...
            print(f"  🎯 Keeping: {segments_to_keep[0]['id'][:8]}...")
...
def main():
    ...
    cleaner = DuplicateSegmentCleaner()
    success = cleaner.run_cleanup()
    
    if success:
        print("\n✅ Duplicate segments cleanup completed")
        sys.exit(0)
    else:
        print("\n❌ Duplicate segments cleanup failed")
        sys.exit(1)
