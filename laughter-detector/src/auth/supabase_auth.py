...
class AuthService:
    ...
    def __init__(self):
        ...
        self.supabase: Client = get_anon_client()
    ...
    async def register_user(
        ...
        try:
            ...
            # If Supabase returns an access token we insert via the user's JWT (RLS enforced).
            # Some environments (email confirmation enabled) return no session; in that case we
            # fall back to a service-role upsert so the profile is guaranteed to exist.
            if response.session and response.session.access_token:
                ...
                if not getattr(settings, "supabase_service_role_key", None):
                    raise HTTPException(
                        ...
                        detail="Service role key not configured; cannot create user profile.",
                    )
            ...
            # Step 4: Ensure we have a live session (useful when email confirmations are disabled).
            session = response.session
            if session is None:
                login_response = self.supabase.auth.sign_in_with_password(
                    {"email": email, "password": password}
                )
                if login_response.user is None or login_response.session is None:
                    raise HTTPException(
                        status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                        detail="Registration succeeded but automatic login failed.",
                    )
            ...
            return {
                ...
                "session": session,  # Contains access_token for authenticated requests
            }
    ...
    async def create_user_profile(
        self, user_id: str, email: str, timezone: str, access_token: str
    ) -> None:
        """
        Ensure the application-level user profile row exists.

        Supabase Auth may not return a session token (e.g., email confirmations, magic links).
        The reliable way to guarantee the profile row is to upsert using the service-role key,
        ...
        Args:
            ...
            access_token: Unused here, kept for interface compatibility.
        """
    ...
    async def enable_mfa(self, user_id: str) -> bool:
        ...
        try:
            ...
            response = service_client.auth.admin.update_user_by_id(
                user_id, {"app_metadata": {"mfa_enabled": True}}
            )
    ...
    async def get_current_user(self, token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            temp_client = get_user_client(token)
            ...
            if user_data.get("id") != user_id:
                print(
                    f"User ID mismatch: JWT user_id={user_id}, DB user_id={user_data.get('id')}"
                )
                raise HTTPException(
                    status_code=status.HTTP_401_UNAUTHORIZED,
            ...
            return {
                ...
                "timezone": user_data.get(
                    "timezone", "UTC"
                ),  # Include timezone for proper date handling
